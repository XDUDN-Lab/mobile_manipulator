...
class RecordDistance():
    ...
    def __init__(self):
        ...
        for i in range(len(robots)):
            ...
            while temp != 0:
                try:
                    self.listener.waitForTransform('/'+ robots[i]+'/odom','/'+ robots[i]+'/base_footprint', rospy.Time(), rospy.Duration(5))
                    temp = 0
                except(tf.Exception):
    ...
    def get_odom(self, robot):
        ...
        try:
            (trans, rot) = self.listener.lookupTransform('/'+ robot+'/odom', '/'+robot+'/base_footprint', rospy.Time(0))
        except (tf.Exception):
